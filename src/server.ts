...
export function createRPDServer() {
  ...
  if (req.method === 'POST' && req.url === '/api/product/extract') {
    try {
      ...
      const qualification = qualifyProduct({ price: product.price.value, mrp: product.mrp.value, discountPercent: product.discountPercent.value, imageCount: product.images.length, title: product.title.value, platform: new URL(product.sourceUrl == null ? body.url.trim() : String(product.sourceUrl)).hostname });
